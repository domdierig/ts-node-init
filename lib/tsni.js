...
const tsni = async () => {
    inquirer_1.default
        ...
        await fs_1.default.writeFile('package.json', JSON.stringify(packagejson), (error) => {
            if (error) {
                throw error;
            }
        });
        await fs_1.default.writeFile('tsconfig.json', JSON.stringify(tsconfigjson_const_1.tsconfigjson), (error) => {
            if (error) {
                throw error;
            }
        });
        if (fs_1.default.existsSync('src')) {
            await fs_1.default.writeFile('src/app.ts', "console.log('hello world');", (error) => {
                if (error) {
                    throw error;
                }
            });
